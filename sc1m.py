# # Experiments with Single Cell data using Spark - 10x Genomics 1 million rows
...
projectRDD = project.map(lambda (id, vec): (id, vec.indices.tolist(), vec.values.tolist()))
schema = StructType([StructField("id", StringType(), False),
                     StructField("idx", ArrayType(IntegerType(), False), False),
                     StructField("quant", ArrayType(DoubleType(), False), False)])
dfProjected = spark.createDataFrame(projectRDD, schema)
dfProjected.write.parquet("celldb1m-projected")

# ## 5. Filter out rows with fewer than 2000 measurements
filtered = rows.filter(lambda vec : len(vec[1].values) >= 2000)
...
#! hadoop fs -rm -r celldb-projected